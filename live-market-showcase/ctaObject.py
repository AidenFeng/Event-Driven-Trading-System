"""
some configurations, objects (classes), etc in CTA module
"""
...
class CtaPortfolioSettings(object):
    """CTA portfolio configuration class"""
    ...
    def from_config_file(self, file=None):
        if file is None:
            file = os.path.join(os.path.dirname(__file__), 'CTA_config.json')  # Json file in the same directory as the cta_config.json file
        with open(file) as f:
            st = json.load(f)
...
class CtaStrategyConfig(object):
    """CTA strategy parameter configuration class"""
