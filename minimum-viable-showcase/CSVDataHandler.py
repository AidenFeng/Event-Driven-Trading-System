...
class CSVDataHandler(DataHandler):
    ...
    def __init__(self, event_queue, file_dir, replay_speed=1):
        ...
        self.__cursor = 0
        self.event_queue = event_queue     # event queue
        self.replay_speed = replay_speed   # replay_speed
        self.run_thread = None             # __run() thread

    def __run(self):
        while self.continue_backtest:
            if self.__cursor == len(self.__data) - 1:   # data run out
                self.continue_backtest = False
            else:
                self.__cursor += 1
            self.event_queue.put(MarketEvent())   # generate MareketEvent
            print('>>>>>>>>>>>>>>>>>>>>> putting MarketEvent <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
            time.sleep(self.replay_speed)   # control playback speed
